...
def main():
    with initialize(config_path="../configs", job_name="run_mlm", version_base="1.3"):
        cfg = compose(
            config_name="config",
        ...
        bert_cfg = instantiate(cfg.model.config, vocab_size=tokenizer.vocab_size)
        model = AutoModelForMaskedLM.from_config(bert_cfg)
        dataset = get_dataset_from_processed()
        def tokenize_funct(examples):
            return tokenizer(examples['text'], padding = True, truncation=True, return_special_tokens_mask = True)
