...
from typing import Dict, List, Sequence, Union, Callable
...
def _build_file_map(root: Union[str, Path], layout: str) -> Dict[str, Sequence[str]]:
    base = Path(root) / "data"
    if layout == "processed":
        proc = base / "processed"
        return { split: [str(proc / f"{split}.txt")] for split in ["train", "val", "test"] }
    elif layout == "raw":
        raw = base / "raw"
        return { split : [
...
def load_dns_dataset(
    ...
    if tokenizer is not None:
        ...
        ds = ds.map(_tokenize, batched=True, remove_columns=["text"], num_proc=4)
    return ds

if __name__ == "__main__":
    with hydra.initialize_config_dir(config_dir=str(Path.cwd() / "configs"), job_name="dataset_test", version_base="1.3"):
